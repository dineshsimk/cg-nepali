...
-- Reflective Pronoun
  ...
  oper
    ...
    detcn2NP : (Determiner) -> Noun -> NPCase -> Number -> Str = \dt,cn,npc,nn -> case npc of {
       ...
       NPErg => dt.s ! dt.n ! Masc ++ cn.s ! nn ! Nom ++ "le"
      } ;  
...
------------------------------------
 ...
 param
  ...
  -- if the noun is occupation/Introductiary case it will take ho, instead of chh (in case of present)   
  oper 
    ...
    infVV :  VPH -> {s:Agr => Str} = \vp -> 
       {
        ...
        } ; -- need to be checked and should be covered in urdu as well
...
--     prog = vp.prog ;
    ...
    insertTrans : VPH -> VType -> VPH = \vp,vtype -> {
      ...
      subj = case vtype of {
             ...
             VTrans   => VTrans ; 
             _        => vtype
             } ; -- still some problem not working properly
      ad  = vp.ad;
      embComp = vp.embComp ;
      --prog = vp.prog ;
      comp = vp.comp
